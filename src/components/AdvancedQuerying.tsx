...
const advancedQueryingOptions = [
  'Multi-Dimensional Data (MDD) Querying',
  'Multi-Valued Data (MVD) Querying',
  'File-based Chat',
  'LLM-assisted Chat'
]

interface AdvancedQueryingProps {
  onOptionSelect: (option: string) => void
  selectedOption: string | null
  onReset: () => void
}

export function AdvancedQuerying({ onOptionSelect, selectedOption, onReset }: AdvancedQueryingProps) {
  return (
    <div className="bg-white shadow-sm rounded-lg p-6 dark:bg-gray-800">
      <h2 className="text-2xl font-semibold mb-4 text-gray-900 dark:text-white">Advanced Querying</h2>
      <div className="flex flex-wrap gap-4 mb-6">
        {advancedQueryingOptions.map((option) => (
          <button
            key={option}
